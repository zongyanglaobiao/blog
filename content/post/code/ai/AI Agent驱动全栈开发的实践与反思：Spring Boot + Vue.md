...
1. 先从 PRD 中提炼需求，与 AI 一起完成需求分析并明确目标，
2. 再沉淀业务架构、技术架构、业务流程、权限设计、领域建模、功能模块设计和数据库表设计等关键文档；这一步是让自己审核明确需求
3. 随后构建适合自己的 Agent 工作流（Hooks、Skills、Rules、ECC、Superpowers），按最小可验证单元拆分任务，单个模块结合 TDD，整体过程结合 SDD，最后通过联调与 E2E 回归测试完成闭环。
...
[Harness Engineering](https://www.runoob.com/ai-agent/harness-engineering.html) 这个概念最开始对我最大的影响，因为阅读 [OpenAI](https://openai.com/zh-Hans-CN/index/harness-engineering/) 文章其中的"其中没有一行代码是人工编写的"打动了我，我也想实现这样的效果，想探索一种针对所有项目的范式框架，但是反而没给我方法，反而是让我一度陷入了新的执念。
